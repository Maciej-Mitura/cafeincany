...
const highlights: Highlight[] = [
  {
    ...
    description: "Van Duvel tot Westmalle, streekbieren en speciale tapbieren. Alles wat een mens nodig heeft om goed te zijn.",
  },
  {
    icon: (
      <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    description: "Elke week themavonden, live muziek en gezellige bijeenkomsten. Van karaoke tot DJ-sets, er is altijd wat te beleven.",
  },
  {
    icon: (
      <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="grid md:grid-cols-3 gap-5 sm:gap-8">
        {highlights.map((highlight, index) => (
          ...
          >
            ...
            >
              <div className="text-[var(--accent)] transition-transform duration-300 group-hover:scale-110">{highlight.icon}</div>
            </div>

            {/* Content */}
            <h3 className="text-2xl font-heading text-[var(--text)] mb-3 transition-colors duration-300 group-hover:text-[var(--accent)]">{highlight.title}</h3>
